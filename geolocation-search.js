...
function getAnimatedIcon(code) {
    ...
    if ([0, 1].includes(code)) {
        return `<svg viewBox="0 0 64 64" class="w-full h-full animate-spin-slow text-yellow-500 overflow-visible">
            <circle cx="32" cy="32" r="14" fill="currentColor" class="animate-pulse shadow-neon-gold" />
            <g class="animate-spin-reverse">
                ${[0, 45, 90, 135, 180, 225, 270, 315].map(rot =>
            `<rect x="30" y="5" width="4" height="8" rx="2" transform="rotate(${rot} 32 32)" fill="currentColor" class="opacity-80" />`
        ).join('')}
            </g>
        </svg>`;
    }
    ...
    if ([2, 3].includes(code)) {
        return `<svg viewBox="0 0 64 64" class="w-full h-full text-gray-300">
            <circle cx="24" cy="24" r="10" fill="#EAB308" class="animate-bounce-slow opacity-60" />
            <path d="M46 48H18c-6.6 0-12-5.4-12-12 0-5.2 3.3-9.6 8-11.3 1.2-6.5 6.4-11.7 13-11.7.9 0 1.9.1 2.8.3 2.6-6.4 8.6-11.3 16.2-11.3 9.4 0 17 7.6 17 17 0 1.5-.2 2.9-.6 4.3 6.1 1.6 10.6 7.1 10.6 13.7 0 7.7-6.3 14-14 14z" fill="currentColor" class="drop-shadow-lg opacity-90 animate-pulse-slow" />
        </svg>`;
    }
    // RAIN (51-65, 80-82)
    if ([51, 53, 55, 61, 63, 65, 80, 81, 82].includes(code)) {
        return `<svg viewBox="0 0 64 64" class="w-full h-full text-blue-400">
             <path d="M46 36H18c-6.6 0-12-5.4-12-12 0-6.6 5.4-12 12-12 1.3 0 2.6.2 3.8.6 2.5-6 8.4-10.6 15.2-10.6 9.4 0 17 7.6 17 17 0 1.5-.2 2.9-.6 4.3 6.1 1.6 10.6 7.1 10.6 13.7 0 6.6-5.4 12-12 12z" fill="gray" class="opacity-60" />
             <g class="animate-rain">
                <line x1="20" y1="40" x2="16" y2="50" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                <line x1="32" y1="40" x2="28" y2="52" stroke="currentColor" stroke-width="2" stroke-linecap="round" class="delay-100" />
                <line x1="44" y1="40" x2="40" y2="50" stroke="currentColor" stroke-width="2" stroke-linecap="round" class="delay-200" />
             </g>
        </svg>`;
    }
    // SNOW (71-77, 85-86)
    if ([71, 73, 75, 77, 85, 86].includes(code)) {
        return `<svg viewBox="0 0 64 64" class="w-full h-full text-white">
            <path d="M46 36H18c-6.6 0-12-5.4-12-12 0-6.6 5.4-12 12-12 1.3 0 2.6.2 3.8.6 2.5-6 8.4-10.6 15.2-10.6 9.4 0 17 7.6 17 17 0 1.5-.2 2.9-.6 4.3 6.1 1.6 10.6 7.1 10.6 13.7 0 6.6-5.4 12-12 12z" fill="gray" class="opacity-40" />
            <g class="animate-snow">
                <circle cx="20" cy="45" r="2" fill="currentColor" />
                <circle cx="32" cy="50" r="2.5" fill="currentColor" class="delay-300" />
                <circle cx="44" cy="45" r="2" fill="currentColor" class="delay-150" />
            </g>
        </svg>`;
    }
    // STORM (95-99)
    if ([95, 96, 99].includes(code)) {
        return `<svg viewBox="0 0 64 64" class="w-full h-full text-purple-400">
             <path d="M46 38H18c-6.6 0-12-5.4-12-12 0-6.6 5.4-12 12-12 1.3 0 2.6.2 3.8.6 2.5-6 8.4-10.6 15.2-10.6 9.4 0 17 7.6 17 17 0 1.5-.2 2.9-.6 4.3 6.1 1.6 10.6 7.1 10.6 13.7 0 6.6-5.4 12-12 12z" fill="#4B5563" />
             <path d="M30 36L22 48H32L28 60L42 44H32L36 36H30Z" fill="#FDE047" class="animate-pulse shadow-neon-gold" />
        </svg>`;
    }
    // FOG (45, 48)
    return `<svg viewBox="0 0 64 64" class="w-full h-full text-gray-400">
        <path d="M10 20h44" stroke="currentColor" stroke-width="4" stroke-linecap="round" class="animate-pulse opacity-50" />
        <path d="M14 32h36" stroke="currentColor" stroke-width="4" stroke-linecap="round" class="animate-pulse opacity-70 delay-100" />
        <path d="M18 44h28" stroke="currentColor" stroke-width="4" stroke-linecap="round" class="animate-pulse opacity-50 delay-200" />
    </svg>`;
}
...
function getMoonPhaseIcon(phaseIndex) {
    const phases = [
        { name: 'Nouvelle Lune', icon: '<div class="w-full h-full rounded-full bg-gray-800 border border-gray-700 shadow-inner"></div>' },
        { name: 'Premier Croissant', icon: '<div class="w-full h-full rounded-full bg-gray-800 border border-gray-700 relative overflow-hidden"><div class="absolute right-0 top-0 bottom-0 w-1/2 bg-gray-100 rounded-r-full opac-80"></div></div>' },
        { name: 'Premier Quartier', icon: '<div class="w-full h-full rounded-full bg-gradient-to-r from-gray-800 to-gray-100 border border-gray-600"></div>' },
        { name: 'Gibbeuse C.', icon: '<div class="w-full h-full rounded-full bg-gray-100 relative"><div class="absolute left-0 top-0 bottom-0 w-1/4 bg-gray-800 rounded-l-full opacity-20"></div></div>' },
        { name: 'Pleine Lune', icon: '<div class="w-full h-full rounded-full bg-gray-100 shadow-[0_0_20px_white] animate-pulse-slow"></div>' },
        { name: 'Gibbeuse D.', icon: '<div class="w-full h-full rounded-full bg-gray-100 relative"><div class="absolute right-0 top-0 bottom-0 w-1/4 bg-gray-800 rounded-r-full opacity-20"></div></div>' },
        { name: 'Dernier Quartier', icon: '<div class="w-full h-full rounded-full bg-gradient-to-l from-gray-800 to-gray-100 border border-gray-600"></div>' },
        { name: 'Dernier Croissant', icon: '<div class="w-full h-full rounded-full bg-gray-800 border border-gray-700 relative overflow-hidden"><div class="absolute left-0 top-0 bottom-0 w-1/2 bg-gray-100 rounded-l-full opac-80"></div></div>' }
    ];
...
function getStarlinkIcon(bars) {
    // Generate a little signal bars SVG
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const color = i <= bars ? (bars > 3 ? "bg-neon-blue shadow-[0_0_5px_#00d4ff]" : "bg-green-400") : "bg-gray-800";
        html += `<div class="w-1.5 rounded-t-sm ${color} transition-all" style="height: ${h}%"></div>`;
    }
    html += '</div>';
    return html;
}
...
function detectAstroPhenomena(date, cloudCover, moonPhase, lat) {
    ...
    // 1. Meteor Showers (Simple Date Ranges)
    if (month === 8 && day >= 10 && day <= 14) {
        list.push({ text: "PERSÉIDES", icon: "☄️", color: "text-purple-400" });
    }
    if (month === 12 && day >= 10 && day <= 15) {
        list.push({ text: "GÉMINIDES", icon: "✨", color: "text-blue-300" });
    }
    if (month === 10 && day >= 6 && day <= 10) { // Draconids
        list.push({ text: "DRACONIDES", icon: "🐉", color: "text-orange-400" });
    }

    // 2. Supermoon
    if (moonPhase === 4) { // Full Moon
        // Toy logic: random "supermoon" for demo or hardcode known dates
        if (month === 9 || month === 10) list.push({ text: "SUPER LUNE", icon: "🌕", color: "text-yellow-200" });
    }

    // 3. Aurora Borealis (High Activity Proxy)
    // If Latitude > 45 (France is ~45-50) and Clear Sky at Night
    if (lat > 45 && cloudCover < 20) {
        // Random chance for "Alert" simulation or check Kp index if API available
        // For now, static placeholder if condition is perfect
        if (Math.random() > 0.8) list.push({ text: "AURORES POSSIBLES", icon: "🌌", color: "text-green-400 animate-pulse" });
    }

    // 4. Planet Alignments (Static Demo)
    if (month === 6) list.push({ text: "ALIGNEMENT PLANÉTAIRE", icon: "🪐", color: "text-pink-400" });
    ...
    try {
        ...
        dashboardRoot.innerHTML = `
            ...
            <div class="flex flex-col gap-4 p-4 h-full overflow-y-auto">
                ...
                <!-- MAIN GRID: 3 COLUMNS -->
                <div class="weather-grid-layout animate-fade-in-up delay-100">
                    ...
                    <div class="photo-conditions-panel">
                        <div class="border-b border-gold/20 pb-3 mb-3">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-2xl">📸</span>
                                <h3 class="text-sm font-bold text-white uppercase tracking-wider">Conditions Photo</h3>
                            </div>
                        ...
                        <div class="space-y-3">
                            <!-- EV Value -->
                            <div class="text-center bg-black/30 rounded-lg p-3">
                                <p class="metric-label mb-1">EXPOSITION</p>
                                <p class="text-4xl font-bold text-gold">EV${photo.ev}</p>
                            </div>

                            <!-- Color Temperature -->
                            <div class="text-center bg-black/30 rounded-lg p-3">
                                <p class="metric-label mb-1">TEMPÉRATURE</p>
                                <p class="text-3xl font-bold text-gold">${photo.kelvin}</p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                <div class="bg-black/30 rounded p-2">
                                    ...
                                    <p class="text-white font-semibold">${formatTime(photo.phases.sunrise)}</p>
                                </div>
                                <div class="bg-black/30 rounded p-2">
                                    <p class="text-gray-400 mb-1">🌇 Coucher</p>
                                    <p class="text-white font-semibold">${formatTime(photo.phases.sunset)}</p>
                                </div>
                            </div>

                            <!-- Timeline Arc -->
                            <div class="w-full h-12 rounded-lg overflow-hidden border border-gold/30">
                                ${getSolarArc(photo.phases, isToday ? now : new Date(dateObj.setHours(12, 0, 0, 0)))}
                            </div>

                            <!-- Moon & Starlink -->
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                <div class="bg-black/30 rounded p-2 text-center">
                                    <p class="text-gray-400 mb-1">Lune</p>
                                    <div class="w-8 h-8 mx-auto mb-1">${moonInfo.icon}</div>
                                    <p class="text-white text-[10px]">${moonInfo.name}</p>
                                </div>
                                <div class="bg-black/30 rounded p-2 text-center">
                                    <p class="text-gray-400 mb-1">Starlink</p>
                                    <div class="w-8 h-8 mx-auto mb-1">${getStarlinkIcon(photo.astro.starlinkBars)}</div>
...
function renderHourlyMini(data, dayIndex) {
    ...
    for (let i = startIndex; i < startIndex + 24 && count < 8; i++) {
        ...
        html += `
            <div class="hourly-mini-card ${isCurrentHour ? 'current' : ''}">
                <p class="text-[10px] text-gray-400 font-semibold">${hour === 0 ? '00h' : hour + 'h'}</p>
                <div class="w-8 h-8 text-xs">${weatherInfo.icon}</div>
                ${precipProb > 0 ? `<p class="text-[10px] text-blue-400">${precipProb}%</p>` : ''}
                <p class="text-sm font-bold ${getTempColorClass(Math.round(temp))}">${Math.round(temp)}°</p>
            </div>
        `;
        count++;
    }
...
function renderForecastHorizontal(data, selectedIdx) {
    ...
    for (let i = 0; i < Math.min(daily.time.length, 10); i++) {
        ...
        html += `
            <div onclick="selectDay(${i})" class="forecast-day-card ${isSelected ? 'selected' : ''}">
                <p class="text-xs font-semibold text-white capitalize">${dayName}</p>
                <p class="text-[10px] text-gray-400">${dayDate}</p>
                <div class="w-12 h-12 my-1">${weatherInfo.icon}</div>
                ${precip > 0 ? `<p class="text-[10px] text-blue-400">💧${precip.toFixed(1)}</p>` : ''}
