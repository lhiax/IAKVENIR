...
async function calculateTrajectory(forceInterrupt = false) {
    ...
    const suggestCorrection = (inputValue, inputElement, feedbackElement) => {
        ...
        for (const [key, cityInfo] of Object.entries(KNOWN_CITIES)) {
            // STRICT CHECK: First letter MUST match to avoid "Artzenheim" -> "Baltzenheim"
            if (normalized.length > 0 && normalized[0] !== key[0]) continue;
...
window.startGuidedTour = async function () {
    ...
    // Disable "Destination Futur" Button to avoid conflict
    ...
    const waitForValidation = (signal, isFinal) => {
        return new Promise((resolve, reject) => {
            ...
            const cleanup = () => {
                ...
                // Re-enable "Destination Futur" Button
                const heroBtn = document.getElementById('btn-hero-access');
                if (heroBtn) {
